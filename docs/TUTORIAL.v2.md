...
Create empty `src/etl/__init__.py` and `src/es_extract/__init__.py`, then:
...
def load_settings(*, dotenv_path: str | None = None) -> Settings:
    ...
    # ... build EsConfig, PaginationConfig (PAGE_SIZE, PIT_KEEP_ALIVE),
    #     RetryConfig, SftpConfig, Settings ...
    return Settings(...)
```
...
mid-scan. The fix is a **consistent point-in-time view** plus a **stable cursor**. The legacy answer
was the Scroll API; Elastic now recommends **PIT + `search_after`**, which is what we use:

- `open_point_in_time(index, keep_alive="5m")` → a lightweight named view (a consistent snapshot).
- stateless `search` calls that `sort` by a stable key and pass `search_after` = the previous page's
  last hit's sort values. Close with `close_point_in_time`.
  - `sort: [{"_shard_doc": "asc"}]` — the cheapest stable tie-breaker, available *only inside a PIT*.
  - `track_total_hits: False` — skip the expensive exact-total computation we don't need (the
    validator gets the authoritative number from `_count`).
  - `keep_alive="5m"` is the TTL ES retains the PIT between calls; each request renews it.

📦 **A standalone package.** The extraction engine lives in `src/es_extract/` — it depends only on the
`elasticsearch` client and the standard library, with **no** import of anything under `etl`, so it can
be copied out or `pip install`ed and reused anywhere. `etl` wraps it to inject its own error type.

📄 **`src/es_extract/pagination.py`** (the one generator):
        ...
        try:
            ...
            while True:
                ...
                if not hits:
                    break
                for h in hits:
                    yield h.get("_source", {}) if self.source_only else h
                last_sort = hits[-1].get("sort")        # the cursor for the next page
                if not last_sort:
                    break
            ...
            if pit_id is not None:
                ...
                except Exception as e:                  # best-effort: the keep-alive reaps it anyway
...
```python
def count(es, index, query, *, error_cls=EsExtractError) -> int:
    try:
...
def iter_hits(es, index, query, *, page_size=1000, keep_alive="5m",
              source_only=True, error_cls=EsExtractError):
    return SearchAfterPagination(keep_alive=keep_alive, source_only=source_only,
                                 error_cls=error_cls).iter_hits(
        es=es, index=index, query=query, page_size=page_size)
```

📄 **`src/etl/extractor.py`** (the thin wrapper that pins failures to `ElasticsearchQueryError`):
...
def iter_hits(es: Any, job: JobSpec, *, page_size: int,
              keep_alive: str = "5m") -> Iterator[dict[str, Any]]:
    return _iter_hits(es, job.data_index, job.query, page_size=page_size,
                      keep_alive=keep_alive, error_cls=ElasticsearchQueryError)
```

🐍 **Python deep-dive:**
- **Resource-owning generators clean up in `finally`.** A PIT is a *server-side resource*. The
  `try/finally` runs `close_point_in_time` **even if the consumer abandons iteration early** (e.g., a
  downstream stage raises and Python closes the generator). This is the make-or-break correctness
  property — forget it and you leak PITs until their keep-alive expires.
- **Swallow cleanup errors.** If `close_point_in_time` itself fails we *log and continue* — raising
  from cleanup would mask the real exception that triggered it.
- **Injectable `error_cls` = decoupling.** `es_extract` wraps every failure in a caller-supplied
  exception type (default `EsExtractError`). The `etl` wrapper passes `ElasticsearchQueryError`, so
  failures land in the daemon's single `EtlError` boundary — while the package itself stays free of
  any `etl` import. Dependency injection applied to the *error taxonomy*.
...
🧪 **Tests — `tests/test_es_extract.py`** (fake ES, assert paging + cleanup-on-early-exit):

```python
from unittest.mock import MagicMock
from es_extract import SearchAfterPagination

def _hit(src, sort): return {"_source": src, "sort": sort}

def test_search_after_pages_and_closes_pit():
    es = MagicMock()
    es.open_point_in_time.return_value = {"id": "pit-1"}
    es.search.side_effect = [
        {"pit_id": "pit-1", "hits": {"hits": [_hit({"n": 1}, [1]), _hit({"n": 2}, [2])]}},
...
def test_search_after_closes_pit_on_early_close():
    es = MagicMock()
    es.open_point_in_time.return_value = {"id": "pit-1"}
    es.search.return_value = {"pit_id": "pit-1",
        "hits": {"hits": [_hit({"n": 1}, [1]), _hit({"n": 2}, [2])]}}
    gen = SearchAfterPagination().iter_hits(es=es, index="i", query={}, page_size=2)
    next(gen); gen.close()              # abandon iteration after one hit
    es.close_point_in_time.assert_called_once_with(body={"id": "pit-1"})  # finally still ran
```

> 💡 Because `es_extract` has no `etl` dependency, you can also exercise it against a *real* cluster in
> isolation: `python scripts/try_es_extract.py --seed --cleanup` seeds a throwaway index, streams it
> back via `count` + `iter_hits`, and checks the streamed total against `_count`.

✅ **Definition of Done:** the pagination tests are green, *especially* the early-`close()` cleanup
test; `es_extract` imports nothing from `etl`; mypy clean.
...
`scripts/prototype.py` (`load_job → expected_count → iter_hits → write_csv`) now
...
- **Inversion of control via a callback.** The validator doesn't know *how* to re-extract — that would
  couple it to the extractor, the pagination, and the CSV writer. It accepts
...
def run_one(*, ctrl: ControlMessage, es: Any, settings: Settings) -> None:
    ...
    csv_result = _do_extract_to_csv(es=es, job=job, page_size=settings.pagination.page_size,
                                    keep_alive=keep_alive, local_csv=local_csv)

    def _reextract() -> CsvResult:                    # closure handed to the validator
        # a fresh call opens a new point-in-time; the previous one is spent
        return _do_extract_to_csv(es=es, job=job, page_size=settings.pagination.page_size,
                                  keep_alive=keep_alive, local_csv=local_csv)
...
(each call opens a fresh point-in-time, so the re-extract is independent of the first). Note `run_one` takes `es`
...
def test_run_one_golden_path(monkeypatch, settings, ...):
    # fake es: get(job doc) -> JobSpec source; count() -> 2; PIT search() -> 2 hits
...
- **Elasticsearch:** deep-pagination limits, PIT + `search_after` (`keep_alive`, `_shard_doc`,
  `track_total_hits`, `search_after` cursor) (C1) · `es.get` by id (C2) ·
