...
(any object exposing ``count`` / ``search`` / ``scroll`` / ``clear_scroll`` /
``open_point_in_time`` / ``close_point_in_time``). It has **no** dependency on
the rest of this repository, so the package can be copied or installed and
reused on its own.
...
from es_extract.pagination import (
    PaginationStrategy,
    ScrollPagination,
